...
public class DrawShear {
	...
	DrawShear(double Rlocal[],String forcetype,Point beamstart,Point beamend, int membernumber,double angle, Point midpoint, double slope, double length){
...
//			 	
	...
	if(Rlocal[1]>0) {
		...
		if(forcetype.matches("UDL")){
			...
			if(Rlocal[4]>0) {
		 		System.out.println(Rlocal[4]+"here");
					Line2D endline = new Line2D.Double(midpoint.x+length*10,  midpoint.y, midpoint.x+length*10, midpoint.y+Rlocal[4]+15);
					g2d.draw(endline);
			 	}else {
			 		Line2D endline = new Line2D.Double(midpoint.x+length*10,  midpoint.y, midpoint.x+length*10, midpoint.y+Rlocal[4]-15);
